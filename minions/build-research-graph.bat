...
set LOG=C:\brain\minions\build-research-graph.log
...
echo [%date% %time%] build-research-graph triggered >> "%LOG%"
"%PYTHON%" "C:\brain\minions\build-research-graph.py" %*
if %errorlevel%==0 (
    echo [%date% %time%] build-research-graph finished OK >> "%LOG%"
) else (
    echo [%date% %time%] build-research-graph FAILED (exit %errorlevel%) >> "%LOG%"
)
endlocal
