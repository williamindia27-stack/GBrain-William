...
set LOG=C:\brain\minions\enrich-researchers.log
...
echo [%date% %time%] enrich-researchers triggered >> "%LOG%"
"%PYTHON%" "C:\brain\minions\enrich-researchers.py"
if %errorlevel%==0 (
    echo [%date% %time%] enrich-researchers finished OK >> "%LOG%"
) else (
    echo [%date% %time%] enrich-researchers FAILED (exit %errorlevel%) >> "%LOG%"
)
endlocal
